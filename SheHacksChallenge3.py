...
def checkForCorrectLetters():
    ...
    global guessesLeft
    global attempt
    for ch in range(len(secretWord)):  # replace blanks with correctly guessed letters
        if secretWord[ch] in userGuess:
            encrypted = encrypted[:ch] + secretWord[ch] + encrypted[ch + 1:]
    print("Word is:", encrypted)
    attempt += 1
    askUserToGuess()
...
def showLettersUsed():
    ...
    global userGuess
    global attempt
    if userGuess.lower() in secretWord and lettersFound == False:
    ...
    if not userGuess.lower() in secretWord and lettersFound == False:
